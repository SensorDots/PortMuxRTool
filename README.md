...
An alternative (and quicker) build method is to get the kernel sources:
```
apt install kernel-source-[kernel version]
```
You can find the available kernel versions with "apt-cache search linux-source" and match it with your running kernel (uname -r). Then extract it out (it should be located in /usr/src), then run the following (you will need sudo apt install build-dep libncurses-dev flex bison):
